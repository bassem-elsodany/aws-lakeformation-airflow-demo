...
resource "aws_glue_job" "event_user_sales_combined" {
  name     = "event_user_sales_combined_job"
  role_arn = module.iam_assumable_role_lakeformation.iam_role_arn
  max_retries = 0

  command {
    script_location = "s3://${aws_s3_bucket.datalake_s3_glue_jobs_bucket.id}/event_user_sales_combined.py"
  }
}